...
2. **生成完一个文件后，明确告知用户**（例如"已生成 `styles/design-system.css`，继续生成图标？"），等待或直接继续下一个文件。
3. **图标文件**：每条消息输出 5 个以内的 SVG 文件；超过时分批输出。
4. **组件文件**：每条消息输出 1–2 个 `components/*.html`，逐一确认再继续。
5. **页面文件**：每条消息输出 1 个 `pages/*.html`。
