...
- 图标文件：每条消息最多 5 个 SVG，超过时分批。
- 组件文件：每条消息最多 2 个 `components/*.html`。
- 页面文件：每条消息 **1 个** `pages/*.html`，不得在同一条消息内生成多个页面。
...
| 6 | 生成页面组件与路由配置 | [visual-hierarchy.md](./references/visual-hierarchy.md)、[final-output-spec.md](./references/final-output-spec.md)、[artifact-structure.md](./references/artifact-structure.md) |
...
读取 [icon-library.md](./references/icon-library.md)，生成 `src/components/icons/` 下的图标 React 组件（TSX）。每个图标接受 `size` 和 `className` props，内联 SVG 路径，使用 `currentColor`。禁止在页面或 UI 组件中临时内联 SVG。
...
- `src/pages/*.tsx`：每个页面一个 TSX 文件，使用 `<Link>`/`useNavigate` 做页面跳转，用 `useState` 控制弹窗
- `src/App.tsx`：注册所有页面路由（Routes / Route）
