...
不要查表对号入座，而是从变更点出发向外追溯：

1. **定位变更点**：这个修改最直接落在哪个文件？（某个组件、某个页面、还是某个 Token 变量？）
2. **判断是否涉及共享层**：如果变更点依赖 `styles/design-system.css` 中的 Token，改 Token 会影响所有引用它的文件，必须逐一确认是否连带受影响。
3. **判断是否影响入口页**：如果新增或删除了文件（图标、组件、页面），对应的总览入口页（`icon-library.html`、`components.html`、`preview.html`）需要同步。
4. **确认边界**：不在影响链上的文件一律不动。
