...
AI 生成 `src/components/ui/` 目录时必须根据产品和页面真实需要选择组件范围。组件视觉必须来自 Design Tokens（通过 Tailwind 类名使用），不得为单个组件临时发明颜色、字号、间距、圆角或阴影。
...
4. 页面组件（`src/pages/`）只能从组件库取组件拼装，不得在页面文件中创建组件库不存在的新样式。
5. 若用户需求出现组件目录未覆盖的场景，优先用 props `variant` 扩展，并在 `src/components/ui/` 中补充该 variant。
...
- Icon 必须从 `src/components/icons/` 中的独立 SVG 文件导入（`import X from '*.svg?react'`），具体规则见 [icon-library.md](./icon-library.md)。
- `src/components/ui/` 中的组件只使用图标，不承担图标总览职责。
- 业务缩写只能作为文字标签出现，不能作为图标本体。例如 `BP` 可以作为指标标签，不能放在图标容器里替代血压图标。
- 图标颜色通过 Tailwind 类名（如 `text-primary`）控制，不硬编码颜色。

图标引用规范以 [artifact-structure.md](./artifact-structure.md) 为准，组件文件不得内联图标 SVG 路径。
...
- 每个 UI 组件是一个独立 TSX 文件，放在 `src/components/ui/` 中。
- 变体通过 props 控制（如 `variant="primary" | "secondary" | "ghost"`），不生成多个同名文件。
- 所有颜色、字号、间距必须使用 Tailwind 类名，来自 `tailwind.config.ts` 中扩展的 Token，不写内联 style。
- 组件库应覆盖本次设计需要复用的组件；不要生成不会被页面使用的组件。
- 页面使用的所有视觉样式，都必须能在 `src/components/ui/` 找到来源。
...
- 图表、指标、趋势等数据展示若出现在页面中，不得临时自造样式，必须作为独立组件放入 `src/components/ui/`。
- 组件必须体现注意力权重差异：高权重卡片、常规卡片、低权重列表项、主按钮、次按钮、标签和元信息不能拥有同等视觉强度。
- 不得出现明显"占位稿"气质：避免随机字母图标、数字图标、emoji 图标、无意义灰块、未命名图片框、重复假内容。
